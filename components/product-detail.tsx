...
export function ProductDetail({ product }: { product: Product }) {
  ...
  return <div className="grid gap-9 lg:grid-cols-2 lg:gap-14">
    <div className="relative grid aspect-[4/5] max-h-[650px] place-items-center overflow-hidden rounded-[18px] bg-mist">{product.badge && <span className="absolute left-5 top-5 z-10 rounded-lg bg-cocoa px-3 py-2 text-xs font-black text-white">{product.badge}</span>}{product.imageUrl?<Image src={assetPath(product.imageUrl)} alt={product.name} fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-contain p-8"/>:<div className="product-bag max-h-[430px] max-w-[330px]" style={{ "--bag": product.color } as React.CSSProperties}/>}</div>
